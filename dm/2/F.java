...
public class F {
	...
	private static void lzw(final int n, final int[] input, final Map<Integer, String> alphabet, final Map<String, Integer> alphabetByString, int pointer, final StringBuilder answer, int prev_index) {
		for (int i = 1; i < n; i++) {
			if (alphabet.containsKey(input[i])) {
				answer.append(alphabet.get(input[i]));
				String s1 = alphabet.get(input[prev_index]);
				final String s2 = alphabet.get(input[i]);
				for (int j = 0; j < s2.length(); j++) {
					final String s3 = s1 + s2.charAt(j);
				...
				final String s1 = alphabet.get(input[prev_index]);
				final String s2 = s1 + s1.charAt(0);
				answer.append(s2);
