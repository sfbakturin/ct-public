...
/**
 * Bank implementation. This class is thread-safe and lock-free using operation objects.
 *
 * This implementation is based on "A Practical Multi-Word Compare-and-Swap Operation" by T. L.
 * Harris et al. It uses a simplified and faster version of DCSS operation that relies for its
 * correctness on the fact that Account instances in [accounts] array never suffer from ABA problem.
...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    /** An array of accounts. Account instances here are never reused (there is no ABA). */
    ...
    override val totalAmount: Long
        get() {
            /**
             * This operation requires atomic read of all accounts, thus it creates an operation
             * descriptor. Operation's invokeOperation method acquires all accounts, computes the
             * total amount, and releases all accounts. This method returns the result.
             */
    ...
    override fun transfer(fromIndex: Int, toIndex: Int, amount: Long) {
        ...
        /**
         * This operation requires atomic read of two accounts, thus it creates an operation
         * descriptor. Operation's invokeOperation method acquires both accounts, computes the
         * result of operation (if a form of error message), and releases both accounts. This method
         * throws the exception with the corresponding message if needed.
         */
    ...
    /**
     * This is an implementation of a restricted form of Harris DCSS operation: It atomically checks
     * that op.completed is false and replaces accounts[index] with AcquiredAccount instance that
     * hold a reference to the op. This method returns null if op.completed is true.
     */
    ...
    /**
     * Releases an account that was previously acquired by [acquire]. This method does nothing if
     * the account at index is not currently acquired.
     */
    ...
    private open class Account(val amount: Long) {
        /**
         * Invokes operation that is pending on this account. This implementation returns false (no
         * pending operation), other implementations return true.
         */
    ...
    private inner class TotalAmountOp : Op() {
        /**
         * The result of getTotalAmount operation is stored here before setting [completed] to true.
         */
        var sum = 0L
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
             ...
             * Basically, implementation of this method must perform the logic of the following code "atomically":
             */
            val firstIndex = Math.min(fromIndex, toIndex)
            val secondIndex = Math.max(fromIndex, toIndex)
            val acquiredFirst = acquire(firstIndex, this)
            val acquiredSecond = acquire(secondIndex, this)

            if (acquiredFirst == null || acquiredSecond == null) {
                release(secondIndex, this)
                release(firstIndex, this)
                return
            }

            if (firstIndex == fromIndex) {
                when {
                    amount > acquiredFirst.amount -> errorMessage = "Underflow"
                    acquiredSecond.amount + amount > MAX_AMOUNT -> errorMessage = "Overflow"
                    else -> {
                        acquiredFirst.newAmount = acquiredFirst.amount - amount
                        acquiredSecond.newAmount = acquiredSecond.amount + amount
                    }
                }
            } else {
                when {
                    amount > acquiredSecond.amount -> errorMessage = "Underflow"
                    acquiredFirst.amount + amount > MAX_AMOUNT -> errorMessage = "Overflow"
                    else -> {
                        acquiredSecond.newAmount = acquiredSecond.amount - amount
                        acquiredFirst.newAmount = acquiredFirst.amount + amount
                    }
