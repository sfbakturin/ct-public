...
public class GenericCheckedParsersizing<T> extends BaseParser {
	...
	private void check() throws ParserException {
		for (int i = 0; i < this.operators.size() - 1; i++) {
			if ((this.operators.get(i) == GenericExpressionType.GENERIC_CONST && this.operators.get(i + 1) == GenericExpressionType.GENERIC_CONST) || (GENERIC_BINARY_OPERATORS_SET.contains(this.operators.get(i)) && GENERIC_BINARY_OPERATORS_SET.contains(this.operators.get(i + 1)))) {
		...
		switch (currentOperator) {
			case GENERIC_ADD, GENERIC_SUBTRACT, GENERIC_MIN, GENERIC_MAX -> {
				switch (currentOperator) {
					...
					case GENERIC_SUBTRACT ->
							result = new GenericSubtract<>(result, this.returnHighestPriority(), this.mode);
