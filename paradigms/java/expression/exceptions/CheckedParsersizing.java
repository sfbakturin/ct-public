...
public class CheckedParsersizing extends BaseParser {
    ...
    private void check() throws ParserException {
        for (int i = 0; i < this.operators.size() - 1; i++) {
            if ((this.operators.get(i) == ExpressionType.CONST && this.operators.get(i + 1) == ExpressionType.CONST)
                    || (BINARY_OPERATORS_SET.contains(this.operators.get(i)) && BINARY_OPERATORS_SET.contains(this.operators.get(i + 1)))) {
