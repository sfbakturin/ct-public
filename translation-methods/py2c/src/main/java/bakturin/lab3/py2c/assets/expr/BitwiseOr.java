...
public class BitwiseOr extends BinaryOperator {
	public BitwiseOr(final CExpression left, final CExpression right) {
		super(left, right);
	}
	...
	protected String binFormat() {
		...
		if (type == CType.STRING || type == CType.FLOAT) {
			throw new Py2CUnsupportedBinaryExpressionOperationException(CExpressionType.BITWISE_OR, type);
		} else {
			return "%s | %s";
		}
	}
