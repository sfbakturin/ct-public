...
public class BitwiseAnd extends BinaryOperator {
	public BitwiseAnd(final CExpression left, final CExpression right) {
		super(left, right);
	}
	...
	protected String binFormat() {
		...
		if (type == CType.STRING || type == CType.FLOAT) {
			throw new Py2CUnsupportedBinaryExpressionOperationException(CExpressionType.BITWISE_AND, type);
		} else {
			return "%s & %s";
		}
	}
