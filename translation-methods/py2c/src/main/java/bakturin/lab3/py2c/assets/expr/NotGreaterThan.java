...
public class NotGreaterThan extends BinaryOperator {
	public NotGreaterThan(final CExpression left, final CExpression right) {
		super(left, right);
	}
	...
	protected String binFormat() {
		...
		if (type == CType.STRING) {
			throw new Py2CUnsupportedBinaryExpressionOperationException(CExpressionType.NGT, type);
		} else {
			return "%s <= %s";
		}
	}
