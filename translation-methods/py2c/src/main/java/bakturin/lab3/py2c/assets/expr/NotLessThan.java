...
public class NotLessThan extends BinaryOperator {
	public NotLessThan(final CExpression left, final CExpression right) {
		super(left, right);
	}
	...
	protected String binFormat() {
		...
		if (type == CType.STRING) {
			throw new Py2CUnsupportedBinaryExpressionOperationException(CExpressionType.NLT, type);
		} else {
			return "%s >= %s";
		}
	}
