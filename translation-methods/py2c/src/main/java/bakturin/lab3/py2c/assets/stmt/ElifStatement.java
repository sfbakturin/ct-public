...
public class ElifStatement implements CStatement {
	private final CExpression expression;
	private final List<CStatement> statements;

	public ElifStatement(final CExpression expression, final List<CStatement> statements) {
	...
	public String asStatement() {
		return "else if" + expression.asExpression() + "{" + System.lineSeparator() + statements.stream().map(CStatement::asStatement).collect(Collectors.joining()) + "}" + System.lineSeparator();
	}

	@Override
	public String asStatement(final Map<String, CVariable> args) {
		throw new Py2CUnsupportedOperationException("compiling with arguments in ElifStatement");
	}

	@Override
	public CType getType() {
		throw new Py2CUnsupportedOperationException("type of ElifStatement");
	}

	@Override
	public String asExpression() {
		throw new Py2CUnsupportedOperationException("expression of ElifStatement");
	}
