...
public class Multiply extends BinaryOperator {
	public Multiply(final CExpression left, final CExpression right) {
		super(left, right);
	}
	...
	protected String binFormat() {
		...
		if (type == CType.STRING) {
			throw new Py2CUnsupportedBinaryExpressionOperationException(CExpressionType.MULTIPLY, type);
		} else {
			return "%s * %s";
		}
	}
